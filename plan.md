...
| Unsupported feature routing | TLS, socket access, binary result mode, server-side cursors, notice handlers, TPC, and full pipeline semantics have either Rust support or an explicit fallback route. | Blocked; explicit unsupported-mode tests cover several gaps including TLS-required ferrocopg connects, but fallback routing is not complete. |
