...
| Unsupported feature routing | TLS, socket access, binary result mode, server-side cursors, notice handlers, TPC, and full pipeline semantics have either Rust support or an explicit fallback route. | Blocked; explicit unsupported-mode tests cover TLS-required ferrocopg connects, socket access, binary execution/cursor modes, server-side/scrollable/withhold cursors, COPY option gaps, notice handlers, and TPC, but fallback routing is not complete. |
