...
impl DbError {
    pub(crate) fn parse(fields: &mut ErrorFields<'_>) -> io::Result<DbError> {
        ...
        while let Some(field) = fields.next()? {
            raw_fields.push((field.type_(), field.value_bytes().to_vec()));
    ...
    /// Returns the original protocol bytes for a diagnostic field.
    ///
    /// PostgreSQL encodes textual diagnostic fields using the active client
    /// encoding. The existing string accessors remain lossy UTF-8 for backward
    /// compatibility; this accessor lets clients decode fields correctly.
    pub fn field_bytes(&self, type_: u8) -> Option<&[u8]> {
        self.raw_fields
            .iter()
            .find_map(|(field_type, value)| (*field_type == type_).then_some(value.as_slice()))
    }
...
mod tests {
    ...
    fn db_error_preserves_non_utf8_field_bytes() {
        ...
        message.put_u8(b'E');
        message.put_u32((fields.len() + 4) as u32);
        message.extend_from_slice(&fields);

        let Message::ErrorResponse(body) = Message::parse(&mut message)
            .expect("error response should parse")
            .expect("error response should be complete")
        else {
            panic!("expected an error response");
        };
        let error = DbError::parse(&mut body.fields()).expect("fields should parse");

        assert_eq!(error.field_bytes(b'M'), Some(b"price \xa4".as_slice()));
        assert_eq!(error.message(), "price \u{fffd}");
    }
}
