...
impl Client {
    ...
    /// Executes simple-query requests concurrently on the connection.
    ///
    /// Polling all request futures together allows tokio-postgres to pipeline
    /// them on the wire while preserving result order.
    pub fn pipeline_simple_query(
        &mut self,
        queries: &[String],
    ) -> Result<Vec<Vec<SimpleQueryMessage>>, Error> {
        let client = &self.client;
        self.connection.block_on(async {
            join_all(queries.iter().map(|query| client.simple_query(query)))
                .await
                .into_iter()
                .collect()
        })
